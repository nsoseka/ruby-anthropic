...
module Anthropic
  # Anthropic::HTTP the http handler
  module HTTP
    include HTTPHeaders

    def get(path:, parameters: nil)
      parse_jsonl(conn.get(uri(path:), parameters) do |req|
        req.headers = headers
      end&.body)
    end

    def post(path:)
      parse_jsonl(conn.post(uri(path:)) do |req|
        req.headers = headers
      end&.body)
    end
    ...
    def uri(path:)
      if amazon_bedrock?
        ...
        # vertex endpoint File.join(@uri_base, path)
      else
    ...
    def configure_json_post_request(req, parameters)
      req_params = parameters.dup
      stream = parameters[:stream]

      if stream.respond_to?(:call)
        req.options.on_data = to_json_stream(user_proc: stream)
        req_params[:stream] = true # Necessary to tell OpenAI to stream.
      elsif stream
